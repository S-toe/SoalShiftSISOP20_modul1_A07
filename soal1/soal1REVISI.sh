...
echo "Region dengan keuntungan paling sedikit adalah:" $hasil1
echo "	"
#=====>soal B	Menampilkan 2 negara bagian (state) yang memiliki keuntungan (profit) paling
#		sedikit berdasarkan hasil soal poin A

awk -F '	' -v region="$hasil1" '$13 == region { c[$11]+=$21 } END { for ( d in c ) { printf "%.4f",c[d]; print "->", d }}' ${now}/Sample-Superstore.tsv | sort -n > hasil2.txt
...
echo "State dengan keuntungan paling sedikit adalah:" $hasil2A "dan" $hasil2B
echo "	"
...
awk -F '	' -v state1="$hasil2A" -v state2="$hasil2B" -v region="$hasil1" '{ if (($11 == state1 || $11 == state2) && $13 == region) { e[$17]+=$21 }} END { for ( f in e ) { printf "%.4f",e[f]; print "->",f }}' ${now}/Sample-Superstore.tsv | sort -n > hasil3.txt 
